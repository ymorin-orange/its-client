...
class GNSSReport:
    ...
    def __post_init__(self):
        fields = {
            ...
            "latitude": {
                "min_inc": math.radians(-90.0),
                "max_inc": math.radians(90.0),
            },
            "longitude": {
                "min_exc": math.radians(-180.0),
                "max_inc": math.radians(180.0),
            },
            "ellipse_orientation": {
                "min_exc": math.radians(-180.0),
                "max_exc": math.radians(180.0),
            },
            "true_heading": {
                "min_exc": math.radians(-180.0),
                "max_exc": math.radians(360.0),
            },
            "magnetic_heading": {
                "min_exc": math.radians(-180.0),
                "max_inc": math.radians(180.0),
        ...
        def _range(f, as_degrees=False):
            ...
            if "min_inc" in fields[f]:
                min_rng += f"[{math.degrees(fields[f]['min_inc']) if as_degrees else fields[f]['min_inc']}"
            elif "min_exc" in fields[f]:
                min_rng += f"]{math.degrees(fields[f]['min_exc']) if as_degrees else fields[f]['min_exc']}"
            max_rng = ""
            if "max_inc" in fields[f]:
                max_rng += f"{math.degrees(fields[f]['max_inc']) if as_degrees else fields[f]['max_inc']}]"
            elif "max_exc" in fields[f]:
                max_rng += f"{math.degrees(fields[f]['max_exc']) if as_degrees else fields[f]['max_exc']}["
        ...
        def _validate(f, v, f_d=None, v_d=None):
            if "min_inc" in fields[f] and v < fields[f]["min_inc"]:
                raise AttributeError(
                    f"{f_d or f} {v_d if f_d else v} is out of range {self._range(f, f_d)}"
                )
        ...
        for f in fields:
            ...
            if rad is not None:
                self._validate(f, rad)
                object.__setattr__(self, f"{f}_d", math.degrees(rad))
            elif deg is not None:
                rad = math.radians(deg)
                self._validate(f, rad, f"{f}_d", deg)
